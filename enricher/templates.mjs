...
const ANGLES = [
  { id: 'ads', when: f => f.runsAds,
    line: (f, p) => `you're renting every lead through paid ads — the day you stop, it stops. I get businesses like yours the same ${p} from Google and AI search at a fraction of that ad spend` },

  { id: 'ai-competitor', when: f => f.score != null && f.score < 58 && f.competitor,
    line: (f, p) => `when your customers ask ChatGPT or Google AI who to use, ${f.competitor} gets named and you don't — that's ${p} handed straight to them. Getting you into that answer is exactly what I do` },

  { id: 'reviews', when: f => f.reviewCount != null && f.compReviews != null && f.reviewCount < f.compReviews * 0.5,
    line: (f, p) => `${f.competitor} shows ${f.compReviews} reviews to your ${f.reviewCount} — that gap alone is why AI and buyers pick them over you. I close it fast and you win those ${p} back` },

  { id: 'slow', when: f => (f.pageSpeed != null && f.pageSpeed < 40) || (f.loadS != null && f.loadS >= 5),
    line: (f, p) => `your site takes ${f.loadS ? f.loadS + 's' : 'far too long'} to load, so you lose over half your visitors before it opens — you're paying for traffic that never sees you. I win those ${p} back` },

  { id: 'no-capture', when: f => !f.hasForm && !f.hasChat && (f.runsAds || (f.score != null && f.score >= 45)),
    line: (f, p) => `you're sending visitors to a homepage with no way to capture them — that traffic leaks straight out. I turn the visitors you already get into ${p}` },
